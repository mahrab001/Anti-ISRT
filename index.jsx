...
const generateAIContent = async (prompt, systemContext = "") => {
  try {
    const apiKey = ""; // Runtime key provided by environment
...
function MobileNavButton({ icon: Icon, active, onClick, hasBadge }) {
  return (
    <button 
      onClick={onClick}
      className={`p-3 rounded-2xl transition-all duration-200 relative ${active ? 'text-indigo-400 bg-indigo-500/10' : 'text-gray-400'}`}
    >
      <Icon size={24} strokeWidth={active ? 2.5 : 2} />
      {hasBadge && (
        <span className="absolute top-2 right-2 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-gray-800 animate-pulse"></span>
      )}
    </button>
  );
}
    ...
    return (
        <div className="px-4">
            <h2 className="text-2xl font-bold mb-6 flex items-center gap-2">
                Messages
                <span className="text-sm font-normal text-gray-500 bg-gray-800 px-2 py-0.5 rounded-full">{chats.length}</span>
            </h2>
            <div className="space-y-3">
                {chats.length === 0 && (
...
function Profile({ appUser, isMyProfile, onViewProfile }) {
    ...
    const handleSaveProfile = async () => {
        ...
        try {
            await update(ref(db, `users/${appUser.uid}`), {
                displayUsername: editDisplayName,
                bio: editBio
            });
    ...
    const handleShowFriends = async () => {
        ...
        // Fetch full friend objects to get UID for linking
        const usersRef = ref(db, 'users');
        const snap = await get(usersRef);
        if (snap.exists()) {
            const allUsers = snap.val();
            const friendsData = Object.entries(allUsers)
                .map(([uid, u]) => ({ uid, ...u }))
    ...
    const nameStyle = appUser.username.toLowerCase() === 'tuntun' 
        ? { color: '#FFD700', textShadow: '0 0 5px rgba(255, 215, 0, 0.5)' } 
        : {};
    ...
    return (
        <div className="min-h-screen pb-10">
                    ...
                    <div className="bg-gray-800 rounded-2xl p-6 w-full max-w-md border border-gray-700 shadow-2xl h-[400px] flex flex-col">
                        <div className="flex justify-between items-center mb-4">
                            <h2 className="text-xl font-bold text-white flex items-center gap-2">
                                <Users size={20} /> My Friends
                            </h2>
                            <button onClick={() => setShowFriendList(false)} className="text-gray-400 hover:text-white">
                                <X size={20} />
                            </button>
                        </div>
                        <div className="flex-1 overflow-y-auto space-y-2">
                            {friendsList.map(friend => (
                                <div 
                                    key={friend.uid} 
                                    onClick={() => { setShowFriendList(false); onViewProfile(friend.uid); }}
                                    className="flex items-center gap-3 p-3 hover:bg-gray-700 rounded-xl cursor-pointer transition"
                                >
                                    <div className="w-10 h-10 rounded-full bg-gray-600 flex items-center justify-center font-bold text-white">
                                        {friend.username[0].toUpperCase()}
                                    </div>
                                    <div>
                                        <div className="font-bold text-white">{friend.displayUsername}</div>
                    ...
                    <div className="bg-gray-800 rounded-2xl p-6 w-full max-w-md border border-gray-700 shadow-2xl">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input 
                                    type="text" 
                                    className="w-full bg-gray-900 border border-gray-600 rounded-lg p-2.5 text-white focus:border-indigo-500 outline-none"
                            ...
                            <div>
                                ...
                                <textarea 
                                    className="w-full bg-gray-900 border border-gray-600 rounded-lg p-2.5 text-white focus:border-indigo-500 outline-none h-24 resize-none"
                                    value={editBio}
            ...
            <div className="relative">
                <div className="h-40 bg-gradient-to-r from-indigo-900 via-purple-900 to-gray-900 rounded-b-3xl mx-[-1rem] md:mx-0 md:rounded-t-2xl relative overflow-hidden">
                    <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-20"></div>
                </div>
                
                <div className="px-4 -mt-16 relative z-10 flex flex-col items-center md:items-start md:flex-row md:gap-6">
                    <div className="w-32 h-32 rounded-full bg-gray-900 p-1.5 shadow-2xl">
                        <div className="w-full h-full rounded-full bg-gradient-to-br from-indigo-600 to-indigo-800 flex items-center justify-center text-4xl font-bold text-white shadow-inner">
                            {appUser.username ? appUser.username[0].toUpperCase() : '?'}
                        </div>
                    </div>
                    
                    <div className="mt-4 md:mt-16 text-center md:text-left flex-1">
                        <h1 className="text-3xl font-bold text-white tracking-tight" style={nameStyle}>{appUser.displayUsername || appUser.username}</h1>
                        <p className="text-indigo-400 font-medium">@{appUser.username}</p>
                        {appUser.bio && (
                            <p className="text-gray-400 text-sm mt-2 max-w-md mx-auto md:mx-0 italic border-l-2 border-indigo-500/50 pl-3">
                                {appUser.bio}
                            </p>
                        )}
                    </div>
                    ...
                    {isMyProfile && (
                        <div className="mt-4 md:mt-16">
                            <button 
                                ...
                                className="bg-gray-800 hover:bg-gray-700 px-5 py-2 rounded-full border border-gray-600 text-sm font-medium transition shadow-sm flex items-center gap-2"
                            >
            ...
            <div className="mt-8 mx-4 bg-gray-800/50 p-4 rounded-2xl border border-gray-700/50 backdrop-blur-sm flex justify-around text-sm text-gray-400">
                <div 
                    className={`text-center ${isMyProfile ? 'cursor-pointer hover:bg-gray-700/50 rounded-lg p-1 transition' : ''}`}
                    onClick={handleShowFriends}
                >
                    <span className="block font-bold text-white text-xl">{appUser.friends ? Object.keys(appUser.friends).length : 0}</span> 
                    Friends
                </div>
                <div className="text-center">
                    <span className="block font-bold text-white text-xl">{posts.length}</span> 
                ...
                <div className="text-center">
                    <span className="block font-bold text-white text-xl">{new Date(appUser.joinedAt || Date.now()).getFullYear()}</span> 
                    Joined
                </div>
            </div>

            <div className="px-4 mt-8 space-y-4">
                <h3 className="font-bold text-xl text-white mb-4 flex items-center gap-2 pb-2 border-b border-gray-800">
                ...
                {posts.length === 0 && (
                     <div className="text-center py-16 text-gray-500 bg-gray-800/30 rounded-2xl border border-dashed border-gray-700 flex flex-col items-center gap-2">
...
// Single Post View for Notification Navigation
function SinglePostView({ postId, appUser, onBack, onViewProfile }) {
    ...
    return (
        <div className="min-h-screen pb-10">
            <div className="relative h-16 bg-gray-800 border-b border-gray-700 flex items-center px-4">
                <button onClick={onBack} className="bg-black/50 p-2 rounded-full text-white hover:bg-black/70">
                    <ArrowLeft size={20} />
                </button>
                <span className="ml-4 font-bold text-white">View Post</span>
            </div>
            <div className="p-4">
                <PostCard post={post} currentUser={appUser} onViewProfile={onViewProfile} />
            </div>
        </div>
    );
}
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser) {
        ...
        // Listen for Notifications status (check read status)
    ...
    <div className="flex flex-col h-screen bg-gray-900 text-gray-100 font-sans overflow-hidden">
      ...
      <header className="flex-none h-16 bg-gray-800 border-b border-gray-700 flex items-center justify-between px-4 z-20 shadow-md">
        <div className="flex items-center space-x-2 cursor-pointer" onClick={() => setView('home')}>
          ...
          <span className="font-bold text-lg tracking-tight hidden sm:block">Anti Isrt</span>
        </div>
        ...
        <div className="flex items-center space-x-4">
          ...
          >
            ...
            <span className="hidden md:block font-medium text-sm" style={appUserProfile.username.toLowerCase() === 'tuntun' ? { color: '#FFD700' } : {}}>{appUserProfile.displayUsername}</span>
          </button>
        </div>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden relative">
        <aside className="hidden md:flex flex-col w-64 bg-gray-800/50 border-r border-gray-700 pt-6 backdrop-blur-sm">
          ...
          <NavButton icon={UserPlus} label="Friends" active={view === 'friends'} onClick={() => setView('friends')} />
          <NavButton icon={MessageSquare} label="Messages" active={view === 'messages'} onClick={() => setView('messages')} hasBadge={unreadMessages} />
          <NavButton icon={Bell} label="Notifications" active={view === 'notifications'} onClick={() => setView('notifications')} hasBadge={unreadNotifs} />
          <NavButton icon={User} label="Profile" active={view === 'profile'} onClick={() => setView('profile')} />
          <div className="mt-auto mb-6 px-4">
             <button 
              onClick={handleLogout}
              className="flex items-center gap-3 text-red-400 hover:text-red-300 hover:bg-red-500/10 w-full px-4 py-3 rounded-xl transition-all font-medium"
             >
               <LogOut size={20} />
               <span>Log Out</span>
             </button>
        ...
        <main className="flex-1 overflow-y-auto bg-gray-900 scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-transparent relative w-full">
          <div className="max-w-2xl mx-auto w-full pb-20 md:pb-8 pt-4 md:pt-8 px-0 md:px-4">
             ...
             {view === 'single_post' && <SinglePostView postId={viewingPostId} appUser={appUserProfile} onBack={() => setView('home')} onViewProfile={handleViewProfile} />}
          </div>
        </main>
      </div>

      <nav className="md:hidden flex-none h-16 bg-gray-800 border-t border-gray-700 flex justify-around items-center px-2 z-30 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
        <MobileNavButton icon={Home} active={view === 'home'} onClick={() => setView('home')} />
        <MobileNavButton icon={UserPlus} active={view === 'friends'} onClick={() => setView('friends')} />
        <MobileNavButton icon={MessageSquare} active={view === 'messages'} onClick={() => setView('messages')} hasBadge={unreadMessages} />
        <MobileNavButton icon={Bell} active={view === 'notifications'} onClick={() => setView('notifications')} hasBadge={unreadNotifs} />
        <MobileNavButton icon={User} active={view === 'profile'} onClick={() => setView('profile')} />
      </nav>
    </div>
  );
}
